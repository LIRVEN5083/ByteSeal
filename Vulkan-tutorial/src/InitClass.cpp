...
bool HelloTriangleApplication::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
    ...
    vkEnumerateDeviceExtensionProperties(device, NULL, &extensionCount, avaliableExtensions.data());
...
HelloTriangleApplication::HelloTriangleApplication(GLFWwindow* window, uint32_t x, uint32_t y){
    this->window = window;
    this->x = x;
    this->y = y;
}
...
void HelloTriangleApplication::loadModel(){
    ...
    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, MODEL_PATH.c_str(), nullptr, true)) {
...
void HelloTriangleApplication::createTextureSampler(){
    ...
    // СГЛАЖИВАНИЕ 4к MSAA 2x пипеу круто классно
...
void HelloTriangleApplication::createTextureImage(){
    ...
    // It's for saving texture image in buffer
    ...
    // Сразу и на подготовку к чтению и записи
...
void HelloTriangleApplication::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format,
    ...
    imageInfo.format = format; // Posible format
...
void HelloTriangleApplication::createDescriptorSetLayout(){
    ...
    layoutInfo.pBindings = bindings.data(); // Link UBO
...
void HelloTriangleApplication::createSyncObjects() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS) {
            throw std::runtime_error("faile to create reneder finished semaphores!");
        }
    }
}
...
void HelloTriangleApplication::drawFrame(){
    ...
    // Fences привязаны к currentFrame, потому что они защищают командные буферы на CPU от перезаписи.
    // Семафоры рендеринга привязаны к imageIndex, потому что они защищают физические фреймбуферы (картинки) на GPU от гонки данных с монитором.

    // Получение следующей картинки
    uint32_t imageIndex;
    VkResult result = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);

    // Изменилось ли размер окна? Равен ли размер картинки аткуальному размеру окна 
    // или развен ли размер картинки актуальному размеру окна при условии что она всёравно выведется
    // или флаг изменения окна true (т.е когда Callback считал изменения окна)
...
void HelloTriangleApplication::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex){
    ...
    // Прибиваем вершины и Индексы к commandBuffer (буфер картиночек) как Иисуса Христа к кресту
    ...
    // Магическое связывание Вершиного Буфера и Буфера Индекса
...
void HelloTriangleApplication::createGraphicsPipeline(){
    MyFile::file_read vertex_shader("../../../src/GLSL_files/BaseModel/BinaryShader/vert.spv");
    MyFile::file_read fragment_shader("../../../src/GLSL_files/BaseModel/BinaryShader/frag.spv");
    auto bindingDescription = Vertex::getBindingDescription();
    auto attributeDescriptions = Vertex::getAttributeDescriptions();

    VkShaderModule vertShaderModule = createShaderModule(vertex_shader.getSource());
    VkShaderModule fragShaderModule = createShaderModule(fragment_shader.getSource());
...
void HelloTriangleApplication::createRenderPass(){
    ...
    dependency.srcSubpass = VK_SUBPASS_EXTERNAL; // <--- ОБЯЗАТЕЛЬНО: Ждем внешних операций (предыдущего кадра)
    dependency.dstSubpass = 0;                   // <--- ОБЯЗАТЕЛЬНО: Применяем к нашему подпассу
...
void HelloTriangleApplication::createSwapChain(){
    ...
    //СКАЧАТЬ ТРОЙНУЮ БУФЕРИЗАЦИЮ ОНЛАЙН
    ...
    createInfo.imageArrayLayers = 1; // Количество блядских картинок
...
void HelloTriangleApplication::createImageViews(){
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        createInfo.viewType = VK_IMAGE_VIEW_TYPE_2D; // дВа Д картинка читать бесплатно
        createInfo.format = swapChainImageFormat;
        
        // RGB подсветка +100 кадров / 1 наносекудну
...
void HelloTriangleApplication::initWindow(){
    ...
    this->window = glfwCreateWindow(this->x, this->y, "Vulkan test", NULL, NULL);
...
void HelloTriangleApplication::createInstance(){
    ...
    appInfo.pApplicationName = "ByteSeal";
    appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
    appInfo.pEngineName = "Nope";
    appInfo.engineVersion = VK_MAKE_VERSION(1, 0, 0);
    appInfo.apiVersion = VK_API_VERSION_1_0;

    // Linkin Vulkan and GLFW
